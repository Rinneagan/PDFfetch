...
class PactApp:
    ...
    def _download_worker(self, url: str, filename: str) -> None:
        ...
        download_id = id(filename) ^ id(url)
        save_dir = self.selected_directory or self.default_download_dir
        save_path = os.path.join(save_dir, filename)

        try:
            self._ui(lambda: self._add_download_to_queue(download_id, filename))
            self.active_downloads[download_id] = {"active": True, "filename": filename}
            ...
            self._ui(lambda: self._download_complete(download_id, filename))
